  ...
  <div>
    ...
    <section class="my-item">
      <main class="input-area">
        <b-input-group prepend="尿蛋白">
          <b-input placeholder="請輸入" v-model="PRO"></b-input>
        </b-input-group>
        <b-input-group prepend="尿糖">
          <b-input placeholder="請輸入" v-model="GLU"></b-input>
        </b-input-group>
      </main>
      <div class="main-area">
        <div>
...
export default {
  ...
  methods: {
    normorlize() {
      ...
      //alert(this.pass);
      const arr = [
        { ...obj, SubSerial: "PRO", Value: +this.PRO },
        { ...obj, SubSerial: "GLU", Value: +this.GLU }
      ];
      return arr;
    },
    async save() {
  ...
  mounted() {
    const items = store.savedExam.filter(s => s.Serial == 38 && s.SubSerial);
    const item = store.savedExam.find(s => s.Serial == 38);
