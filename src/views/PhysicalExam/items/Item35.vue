<template>
  <div>
    <header class="header-row">
      <div>愛滋</div>
      <b-check switch size="lg" v-model="pass" value="1" class="mt-1">陰性</b-check>
      <b-check switch size="lg" v-model="pass" value="2" class="mt-1">陽性</b-check>
...
export default {
  ...
  mounted() {
    const item = store.savedExam.find(s => s.Serial == 35);
    if (item) {
      this.item = item;
